"""
enrich_price_drogaraia.py — Backfill the real promotional price for Drogaraia.

The fast category listing only exposes `priceService`, which for discounted
products is the *regular* ("de") price — the real selling price ("por") is only
on the product page:
    __NEXT_DATA__ → props.pageProps.productData.price_aux
        value_from  -> regular price
        value_to    -> promotional / selling price

The scraper already stores value_from as regular_price and flags is_discounted
(from stripeCode), but leaves promo_price NULL. This pass fetches the product
page for every is_discounted offer and writes:
    regular_price = value_from   (authoritative)
    promo_price   = value_to     (when value_to < value_from)
    discount_pct  = derived

Usage:
    python -m markets.drogaraia.enrich_price_drogaraia               # full pass
    python -m markets.drogaraia.enrich_price_drogaraia --limit 500   # test run
    python -m markets.drogaraia.enrich_price_drogaraia --workers 20  # faster
"""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Backfill promotional prices for Drogaraia")
